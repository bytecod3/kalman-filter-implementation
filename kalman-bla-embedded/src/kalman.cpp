...
// initial estimate values -> change according to the launch conditions 
...
struct FilteredValues kalmanUpdate(float x_accel, float y_accel) {
    struct FilteredValues return_val;

    // Measurement matrix
    BLA::Matrix<2, 1> Z = {x_accel, y_accel};
    ...
    BLA::Matrix<3, 2> K = P_minus * (~H) * Inverse(con);
    ...
    return_val.x_accel_filtered = x_hat(1);
    return_val.y_accel_filtered = x_hat(2);
